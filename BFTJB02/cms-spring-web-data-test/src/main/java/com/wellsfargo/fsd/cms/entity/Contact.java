...
public class Contact {
	...
	public Contact(
			@NotNull(message = "Contact Id is requried") @Min(value = 1, message = "Contact Id cna not be zero or negative") Integer contactId,
			@NotNull(message = "Full Naem is requried") @NotBlank(message = "Full Naem is requried") @Size(min = 5, max = 25, message = "Full Naem is requried to be of 5 to 25 chars in length") String fullName,
			@PastOrPresent(message = "Date Of Birth can not be a future date") LocalDate dateOfBirth,
			@NotNull(message = "Mobile Number is requried") @NotBlank(message = "Mobile Number is requried") @Pattern(regexp = "[1-9][0-9]{9}", message = "Mobile Number should be of 10 digits only") String mobile,
